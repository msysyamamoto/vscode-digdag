...
export class DigdagAutocomplete implements CompletionItemProvider {
    ...
    provideCompletionItems(document: TextDocument, position: Position, token: CancellationToken): Promise<CompletionItem[]> {
		...
		const result = DigdagAutocomplete.makeProposals(prefix, DigdagAutocomplete.keywords, CompletionItemKind.Keyword)
			.concat(
				DigdagAutocomplete.makeProposals(prefix, DigdagAutocomplete.variables, CompletionItemKind.Variable),
				DigdagAutocomplete.makeProposals(prefix, DigdagAutocomplete.operators, CompletionItemKind.Operator)
			);
